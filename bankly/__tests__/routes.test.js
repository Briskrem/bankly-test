...
beforeEach(async function() {
  ...
  for (let user of sampleUsers) {
   ...
   // this creates 3 tokens for each user inside the for loop.
...
describe("GET /users/[username]", function() {
  ...
  // this test returns 404  as the throw error was programmed to do because throw was added otherwise it was returning
  //200 
  test('not sure what it will return', async ()=>{
    const response = await request(app).get('/users/wrong-user').send({ _token: tokens.u3 })
...
describe("PATCH /users/[username]", function() {
  ...
  test("should patch data if admin", async function() {
    ...
    // });
    //[THE FIX] how can i run test to prove its a bug and fix the code at the same time??
    expect(response.body.user).toEqual({
        username: "u1",
        first_name: "new-fn1",
        last_name: "ln1",
